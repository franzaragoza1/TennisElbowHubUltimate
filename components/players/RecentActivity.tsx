...
const OUTCOME_LABEL: Record<Exclude<RecentActivityMatch["outcome"], "played">, string> = {
  walkover: "w.o.",
  retired: "ret.",
  disqualified: "DISQ",
  random: "RL",
 ...
 * lista a media altura — mejor no decir nada que insinuar algo que no consta.
 */
function tournamentSummary(matches: RecentActivityMatch[]): string | null {
...
function ScoreCell({ scores, outcome }: { scores: PairedSetScore[]; outcome: RecentActivityMatch["outcome"] }) {
...
export function RecentActivity({ groups }: { groups: TournamentActivityGroup[] }) {
  if (groups.length === 0) {
    return <p className="text-muted-label rounded-lg border border-rule bg-paper px-4 py-8">No matches on record yet.</p>;
  }

  return (
    <div className="flex flex-col gap-4">
      {groups.map((g) => (
        <TournamentGroup key={g.editionId} group={g} />
      ))}
    </div>
  );
}
