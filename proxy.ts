...
export const config = {
  ...
  // MatchLog, el callback de Discord, el sondeo de Live Scores...), nunca `/admin`
  // (panel del propietario, no de un jugador nuevo), nunca `/welcome` (o el redirect
  // se muerde la cola), y nunca los archivos estáticos de Next ni los de la raíz de
  // `public/` que el navegador pide solo.
  matcher: ["/((?!api|_next/static|_next/image|admin|welcome|favicon.ico|robots.txt|sitemap.xml).*)"],
};
